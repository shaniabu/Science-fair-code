...
iteration_range = range(1,11)
import matplotlib.pyplot as plt
plt.plot(iteration_range, scores)
plt.xlabel('iteration number')
plt.ylabel('Cross-Validated Accuracy')
plt.show()

print('\nCV accuracy: %.3f +/- %.3f' % (np.mean(scores), np.std(scores)))
#CV accuracy: 0.719 +/- 0.009
...
p_class, r_class, f_class, support_micro=precision_recall_fscore_support(
                y_true=y_test, y_pred=y_pred, labels=[0, 1, 2, 3, 4, 5], average=None)
print('各類單獨F1:',f_class)
print('宏F1：',f_class.mean())
#各類單獨F1: [0.95356037 0.         0.         0.         0.73033708 0.        ]
#宏F1： 0.2806495750281189
